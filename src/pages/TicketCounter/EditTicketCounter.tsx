...
export default function EditTicketCounter() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [userData, setUserData] = useState<TicketCounterUser>({
    id: "",
    name: "",
    email: "",
    password: "",
  ...
  const fetchUser = async () => {
    try {
      const response = await axios.get(
        `${config.baseURL}/ticketcounter/${id}`,
      ...
      setAlertMessage("Error fetching ticket counter user");
  ...
  useEffect(() => {
    ...
    // Test decryption with the exact encrypted string from your API
    console.log("=== MANUAL DECRYPTION TEST ===");
    const testEncrypted = "U2FsdGVkX19mrR0CEySrkTvATIZYYP2tc6cPD4fhFbPhzWiRw0e2E/dv3evlZ/zmf9SWdBdtT7+Jlbb7HlSArA==";
    const testResult = decryptData(testEncrypted);
  ...
  const handleChange = (field: keyof TicketCounterUser, value: any) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setAlertMessage("Ticket counter user updated successfully");
        setAlertVariant("success");
        setTimeout(() => navigate("/ticket-counter"), 1500);
      } else {
        setAlertMessage(
          response.data.message || "Failed to update ticket counter user"
        );
        setAlertVariant("error");
      }
    } catch (err: any) {
  ...
  return (
    <ComponentCard title="Edit Ticket Counter User">
      <div className="space-y-6">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => handleChange("status", e.target.value as "enabled" | "disabled")}
