...
const AddFacilityPage: React.FC = () => {
  ...
  const [categoryKn, setCategoryKn] = useState("");
  ...
  const handleUpload = async () => {
    if (!categoryEn || !categoryKn || !titleEn || !titleKn || !imgFile) {
      setAlertMessage("Please fill all required fields and add image");
    ...
    try {
      ...
      formData.append("category_kn", categoryKn);
      formData.append("title_en", titleEn);
      formData.append("title_kn", titleKn);
      ...
      await axios.post(`${config.baseURL}/facilities`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
        withCredentials: true,
      });

      setAlertMessage("Facility added successfully");
      ...
      setAlertMessage(error.response?.data?.message || "Error adding facility");
      setAlertVariant("error");
    } finally {
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto space-y-6">
      <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Add Facility</h1>

      {alertMessage && (
        <Alert variant={alertVariant} title={alertVariant === "success" ? "Success" : "Error"} message={alertMessage} showLink={false} />
      )}
      ...
      <div className="flex justify-end">
        ...
        >
          {isUploading ? "Uploading..." : "Add Facility"}
        </button>
      </div>
    </div>
  );
